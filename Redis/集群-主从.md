...
void syncWithMaster(connection *conn) {
    ...
    // 当P(增量)Sync不成功时,需要使用(全量)Sync方式(有可能是master不支持,也有可能是master判断需要先sync一份base数据),即向master发送SYNC命令
...
>注:这里的‘sync'指的是调用内核的同步与异步,不是指master接收到信息并返回结果
...
// src/replication.c
// 接收来自slave的replconf命令只是设置了对应连接的信息
...
void syncCommand(client *c) {
    // 如果发请求的连接已经是slave了直接忽略掉,TODO,什么时候这个flag变成了CLIENT_SLAVE?
    if (c->flags & CLIENT_SLAVE) return;

    // 当我们自己本身是别的redis的slave,且还没完成与自己master的连接时,暂时不满足slave的sync要求
    ...
    if (!strcasecmp(c->argv[0]->ptr,"psync")) {
        // 尝试增量 psync,主从的slave发出的“sync”命令都是“psync”,然后master见机行事决定看是否能直接“psync”,不能则需要走“sync(resync)”流程
        if (masterTryPartialResynchronization(c) == C_OK) {
            server.stat_sync_partial_ok++;
            return; // 成功的增量psync了一次,直接返回就好
        } else {
            char *master_replid = c->argv[1]->ptr;
            // 尝试增量psync没有成功,需要统计下err次数
            if (master_replid[0] != '?') server.stat_sync_partial_err++;
        }
    } else {
        // 全量sync,先设置flag,
        // 全量sync了第一批基准数据后,最终还是要走增量psync的,所以flag名叫“PRE_PSYNC”
    ...
    // 设置连接的状态为“等待BGSAVE”,这个BGSAVE操作有点耗时
    c->replstate = SLAVE_STATE_WAIT_BGSAVE_START;
    // ...
    // 设置slave连接的一些属性
    c->repldbfd = -1;
    c->flags |= CLIENT_SLAVE;
    // 将当前连接驾到全局slaves 的 List中
    listAddNodeTail(server.slaves,c);

    // 全量同步需要先在本地创建一个replication back log
    ...
    // 启动bgsave(子进程)全量备份当前数据

    // 如果bgsave子进程已经启动了,且模式是往磁盘里写
    if (server.rdb_child_pid != -1 &&
        server.rdb_child_type == RDB_CHILD_TYPE_DISK)
    {
       ...
       // 如果已经有其他slave发起过bgsave,且正在执行中,且和当前请求成为slave的服务的设置信息一致
...
int masterTryPartialResynchronization(client *c) {
    ...
    // 比较slave传来的replid 和 当前master的replid,满足要求才能继续走psync流程
    ...
    // slave传来的当前offset所包含的数据在master已经不存在了,走不通psync流程
    ...
    // 根据slave传来的offset,从master的replicationBacklog取出这个offset后面的数据写入要发送的连接里
...
int startBgsaveForReplication(int mincapa) {
    ...
    // 当rdb直接写入socket时,还需要给每个slave一些操作
    if (!socket_target) {
        listRewind(server.slaves,&li);
        while((ln = listNext(&li))) {
...
int rdbSaveToSlavesSockets(rdbSaveInfo *rsi) {
    ...
    // 创建一个用于rdb读写的管道,子进程生成rdb数据传给父进程,父进程再把数据传给slave(因为需要父进程和slave保持tls连接)
    if (pipe(pipefds) == -1) return C_ERR;
    server.rdb_pipe_read = pipefds[0];
    server.rdb_pipe_write = pipefds[1];
    anetNonBlock(NULL, server.rdb_pipe_read);

    // 遍历当前master的所有slave连接,设置slave的repl状态(SLAVE_STATE_WAIT_BGSAVE_START)
    ...
    if ((childpid = redisFork()) == 0) {
        ...
        int retval;
        rio rdb;
        // 将子进程的写fd 设置为前面创建的 server.rdb_pipe___ 管道
        rioInitWithFd(&rdb,server.rdb_pipe_write);

        // 向管道里写入全量信息(因为需要直接传给slave,需要补充一些信息,便于slave分步校验和使用得到的rdb数据)
        // rdbSaveRioWithEOFMark 是对 rdbSaveRio(disk模式也是使用这个函数保存rdb数据,只是保存到了不同的fd里) 的封装
        ...
        if (childpid == -1) {
            ...
            // 给redis的主事件循环注册一个读事件处理,当收到子进程通过rdb管道传来的数据时调用 rdbPipeReadHandler 将数据转发给slave
...
```c
// src/rdb.c
// 这个方法 diskless模式直接将rdb数据写给slave的socket,需要在数据前加些标记如“$EOF”包裹原始rdb数据,便于slave收到消息后处理
int rdbSaveRioWithEOFMark(rio *rdb, int *error, rdbSaveInfo *rsi) {
    ...
    if (rioWrite(rdb,"\r\n",2) == 0) goto werr;
    // 调用rdbSaveRio将本地rdb数据写入上一层调用房设置好的写pipeFd(这些数据被pipe的另一段消费并写给slave连接)
...
void rdbPipeReadHandler(struct aeEventLoop *eventLoop, int fd, void *clientData, int mask) {
    ...
    while (1) {
        ...
        if (server.rdb_pipe_bufflen == 0) {
            // 收到rdb全量数据写方发来的“写完”通知
            int stillUp = 0;
            // 解除rdb_pipe_read的可读事件回调
        ...
        {
            // 在方法调用的上一层,已经将server.rdb_pipe_conns[] 设置为了slave的连接,所以可以直接遍历这些连接,将rdb信息写进去
            int nwritten;
            ...
            // 写的数据没有从pipe里读的数据多,需要设置一个write回调,便于下次循环将没写完的数据写给slave
...
int rdbSaveBackground(char *filename, rdbSaveInfo *rsi) {
    pid_t childpid;
    // ...
    // 创建父子进程的消息通道,子进程执行rdbsave完成后需要通知父进程
    openChildInfoPipe();
...
int rdbSave(char *filename, rdbSaveInfo *rsi) {
    ...
    // 将rdb的写入fd设置为刚刚打开的文件的socket
    rioInitWithFile(&rdb,fp);
    startSaving(RDBFLAGS_NONE);

    // 在这里rdbSaveRio实际将数据写入fd
...
```c
// src/replication.c
// 给slave发送一个FullResyc信息,告诉slave需要全量同步
...
### cron相关
redis的cron循环发现后台进程(如rdbsave进程)完成时,调用响应的函数执行“同步”功能的下一步操作
```c
// src/server.c
int serverCron(struct aeEventLoop *eventLoop, long long id, void *clientData) {
    // ...
...
void backgroundSaveDoneHandler(int exitcode, int bysignal) {
    ...
    case RDB_CHILD_TYPE_DISK: // 保存在本地文件的执行分支
        backgroundSaveDoneHandlerDisk(exitcode,bysignal);
        break;
    case RDB_CHILD_TYPE_SOCKET: // 直接通过socket传给slave的执行分支
...
void backgroundSaveDoneHandlerSocket(int exitcode, int bysignal) {
    ...
    // 同bgsaveToDisk一样最后都是调用的 updateSlavesWaitingBgsave
    ...
    while((ln = listNext(&li))) {
        ...
        if (slave->replstate == SLAVE_STATE_WAIT_BGSAVE_START) {
            ...
            if (type == RDB_CHILD_TYPE_SOCKET) {
                // ... diskless模式是直接边生成rdb数据边发给发给slave,不需要做更多操作
                slave->replstate = SLAVE_STATE_ONLINE;
                slave->repl_put_online_on_ack = 1;
                slave->repl_ack_time = server.unixtime; /* Timeout otherwise. */
            } else {
                // 非diskless模式,将rdb数据先完全保存到了本地文件中,然后发送给slave
                ...
                // 设置slave的可写事件为sendBulkToSlave,这样redis的主循环就会开始分批把rdb数据传到slave
